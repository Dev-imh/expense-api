...
''';
  ...
  static Worksheet? _addlistSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _addlistSheet = await _getWorkSheet(spreadsheet, title: 'AddList');

      final firstRow = AddListFileds.getFields();
      _addlistSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<int> getRowCount() async {
    if (_addlistSheet == null) return 0;
    final lastRow = await _addlistSheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_addlistSheet == null) return;

    _addlistSheet!.values.map.appendRows(rowList);
  }
}
